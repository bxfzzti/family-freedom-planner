...
- [x] 安装到默认Codex Skills目录，用gpt-5.5新会话验证隐式调用；复现未触发与触发后跳过问诊两类失败。
- [x] 增加agents/openai.yaml与顶部首次回复门禁；第三次gpt-5.5新会话自动读取Skill和问诊参考，正确给出三项首次问诊。
- [x] 发布v1.3.3代码提交e828dde9329e9d38c8700f78e23812201e69fe7c；远端全新隔离安装通过122项测试和自检，GitHub CI成功，关键文件与默认安装哈希一致。
