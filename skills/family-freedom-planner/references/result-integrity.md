...
rollback 根据实际 workflow 依赖自动失效下游，并删除受影响的旧完整性记录。STALE 步骤会重新出现在合法下一步中。新增校验会撤销旧门禁与最终输出；最终交付前再次核对完整性和推荐门禁。

这些机制检查数据一致性与流程，不认证模型填写的事实或外部审阅身份。仍需实际计算和语义审阅，不能自行填写“PASS”代替执行。

v1.3.2 在记录基线校验时，控制器会自行读取当前步骤结果，复算净资产与年结余恒等式。外部报告即使写 PASS，也不能覆盖实际复算失败；缺少复算所需字段只能条件化通过。领域标签必须与对应步骤一致。
